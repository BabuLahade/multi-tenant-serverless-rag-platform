...
resource "aws_api_gateway_deployment" "rag" {
  ...
  depends_on = [
    ...
    aws_api_gateway_method.onboard_options,
    aws_api_gateway_integration.onboard,
    aws_api_gateway_integration.onboard_options,
    aws_api_gateway_method_response.onboard_options,
    aws_api_gateway_integration_response.onboard_options
  ]
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.onboard.path_part,
      aws_api_gateway_method.onboard_post.http_method,
      aws_api_gateway_method.onboard_post.authorization,
      aws_api_gateway_integration.onboard.uri,

      aws_api_gateway_method_response.onboard_options.response_parameters,
      aws_api_gateway_integration_response.onboard_options.response_parameters,
...
resource "aws_lambda_permission" "apigw_onboard" {
  statement_id  = "AllowAPIGatewayOnboard"
  action        = "lambda:InvokeFunction"
  function_name = var.onboard_lambda_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.rag_api.execution_arn}/*/*"
}
...
resource "aws_api_gateway_method_response" "onboard_options" {
  rest_api_id = aws_api_gateway_rest_api.rag_api.id
  resource_id = aws_api_gateway_resource.onboard.id
  http_method = aws_api_gateway_method.onboard_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "onboard_options" {
  rest_api_id = aws_api_gateway_rest_api.rag_api.id
  resource_id = aws_api_gateway_resource.onboard.id
  http_method = aws_api_gateway_method.onboard_options.http_method
  status_code = "200"
...
# Attach CORS headers to API Gateway default error responses (e.g., 429 Throttled)
